...
describe "Decimal subtraction" do
  ...
  describe "finishing up subtraction" do
    ...
    it "should produce the correct result for 'straight' subtraction" do
      DuckInterpreter.new("1.1 2.2 -").run.stack[-1].value.should be_within(0.0001).of(-1.1)
    end
    
    it "should produce the expected result for 'infix' addition" do
      DuckInterpreter.new("-11.11 - -22.22").run.stack[-1].value.should be_within(0.0001).of(-11.11)
    end
    
    it "should even work if the closure forms first" do
      DuckInterpreter.new("- 17.17 22.22").run.stack[-1].value.should be_within(0.0001).of(5.05)
    end
  end
  ...
  describe "delayed subtraction" do
    it "should work over complex sequences" do
      ...
      ducky.run
      ducky.stack[0].value.should be_within(0.001).of(0.1)
    end
    ...
    it "should leave unasked-for argumnts intact" do
      ...
      d.stack.inspect.should == "[1.1, 2.2, 3.3, -1.0]"
    end
  end
end