...
describe "various bad behaviors that cropped up during stress-testing" do
  it "should be able to deal with Bad Script 1" do
    d = DuckInterpreter.new("F x swap know? ≤ ( eql pop pop x -7 x ( T know? ) inc -3 -8 8 F copy push 4 x neg ≥ -3 greedy 6 ≤ < 7 depth T ¬ F + -8 x -8 do -8 greedy? ≤ / pop -2 x > + -7 ≤ T ≤ ( pop ∨ - do dec 9 ( x 3 4 know? push T F F if + 9 x ∧ 8 -2 / -1 unshift < -5 x - x ( x > F 8 x - F know? x T push neg if F -7 x T eql 4 9 ( T ) T ungreedy > x x ( x - x x x ≥ x push know? push < -7 T x x ) copy F do ∧ -4 x 9 if F neg ≤ copy F eql x -8 ∧ x unshift x x x > F x copy inc > swap dec greedy / 6 -7 -10 be -7 x push ≤ x 0 7 -4 1 ∨ dec -4 x do T F x ungreedy F F be x x copy T neg < x 1 T < shatter x x x -1 ( push -4 push -2 x 8 + 0 x 6 swap copy inc T T 0 ∨ x eql pop", {"x" => Int.new(8)})
  ...
  it "should be able to deal with Bad Script 3" do
    d = DuckInterpreter.new("unshift x 1.379 depth T -6.819 -4 pop x known < T 8 neg - x T x x -4.156 []= T F * pop if ungreedy 8.593 []= 5.353 push F unshift 1.184 neg x 8 7.707 swap F swap x x -2 ungreedy x 6 x x -5",{"x" => Int.new(8)})
  ...
  it "should be able to deal with Bad Script 4" do
    d = DuckInterpreter.new("eql empty x ( -3.777 - < push give 4.214 known ≥ / T eql < x -6.333 []= copy be 5 push if zap ∧ 2.496 pop x know? -3 dec T F x depth F empty F greedy unshift x -7 ∧ greedy? -8 -6 x greedy copy",{"x" => Int.new(8)})
